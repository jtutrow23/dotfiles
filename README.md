...
## 🔁 Legacy: `znap.sh` Replaced with Modular Structure

The previous `znap.sh` setup has been fully modularized:

- `zsh/.zshrc` loads all modular files
- `zsh/init.sh` replaces `znap.sh` for plugin setup
- `zsh/functions.sh` includes prompt toggle (`use_pure`, `use_starship`)
- `zsh/aliases.sh` includes useful CLI shortcuts

Everything is loaded by default when launching a new shell via `.zshrc`.

---

### 🧠 If You Previously Used `znap.sh`

No need to source it anymore. Just use:

```zsh
source ~/.zshrc
```

Or let your terminal session handle it automatically at launch.
