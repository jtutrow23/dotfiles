...
3. **Dotfiles directory** – This guide assumes your dotfiles live in `/Users/jtutrow23/dotfiles` (i.e. `$HOME/dotfiles`).
...
You want your Zsh configs under `~/.config/zsh` while keeping the repository in `~/dotfiles`. Create the directory tree once:

```sh
mkdir -p ~/.config/zsh ~/dotfiles
```
...
*(If you track dotfiles via symlinks, commit these loader files in `~/dotfiles` and symlink them back into `$HOME`.)*

---

## 5. Main Zsh Configuration (`~/.config/zsh`)

Create the following files inside `~/.config/zsh`. Keep them in your repo (e.g. `~/dotfiles/zsh/…`) and symlink into place if desired.
...
if [[ -r "$HOME/dotfiles/znap/znap.zsh" ]]; then
  source "$HOME/dotfiles/znap/znap.zsh"
else
  command -v git >/dev/null 2>&1 \
    && git clone --depth 1 https://github.com/marlonrichert/zsh-snap.git "$HOME/dotfiles/znap" \
    && source "$HOME/dotfiles/znap/znap.zsh"
fi
...
Put the following in `~/dotfiles/bootstrap.sh`, make it executable (`chmod +x bootstrap.sh`), and run it (`~/dotfiles/bootstrap.sh`). The script safely repeats the manual steps above, so you can use it on fresh machines.
...
- Track all configuration files in `~/dotfiles` and symlink them into `$HOME` (e.g. via a small install script or GNU Stow).
